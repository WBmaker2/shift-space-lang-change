...
## 목표 (역사적 기존 계획 — 2026-09-01 결정으로 대체됨)

다음은 공개 선행 결정 전의 역사적 목표 문구이며 현재 활성 게이트가 아니다. 남아 있는 전역 단축키 오류 분류 결함을 수정하고, 실제 Windows CI에서 실행 파일과 NSIS 설치 파일을 만든 뒤 Windows 실기기 HVC를 통과한 경우에만 `main` 병합과 `v0.1.0` 릴리스를 완료한다.

## 사용자 승인 (역사적 기존 계획 — 2026-09-01 결정으로 대체됨)

아래 내용은 2026-08-31 당시 승인 기록이며, 2026-09-01 공개 선행 결정으로 대체되어 현재 승인 순서가 아니다.

- 사용자는 2026-08-31에 추천 순서 전체 진행을 승인했다.
- 당시 승인된 순서: 오류 수정 → Windows 패키지 CI → Windows HVC → `main` 병합 및 `v0.1.0` 릴리스.
- 당시 원격 저장소가 없으므로 소스 노출을 피하는 안전한 기본값으로 `WBmaker2/shift-space-lang-change` 비공개 저장소를 먼저 사용하고 공개 전환은 별도 승인 대상으로 두었다.
...
### 5. 병합·태그·릴리스 게이트 (역사적 기존 계획 — 2026-09-01 결정으로 대체됨)

아래는 공개 선행 결정 전의 역사적 병합·태그·릴리스 게이트이며 현재 활성 게이트가 아니다. 현재 순서는 문서 상단의 `2026-09-01 공개 선행 릴리스 결정`을 따른다.

- 당시 계획: HVC 전체 통과 전에는 `main` 병합, `v0.1.0` 태그, GitHub Release를 수행하지 않는다.
- 당시 계획: HVC 통과 후 기능 브랜치를 `main`에 병합하고 다시 CI를 확인한다.
- 당시 계획: `v0.1.0` 태그를 push하고 생성된 설치 파일을 GitHub Release에 첨부한다.
- 당시 계획: 최종 보고서에는 릴리스 주소와 설치 파일 다운로드 주소를 클릭 가능한 링크로 제공한다.
...
## 완료 조건 (역사적 기존 계획 — 2026-09-01 결정으로 대체됨)

다음은 공개 선행 결정 전의 역사적 완료 조건이며 현재 활성 완료 조건이 아니다. 현재 공개 전 선행 게이트와 공개 후 HVC 상태는 문서 상단의 결정 및 진행 기록을 따른다.

1. 당시 조건: 오류 분류 수정이 독립 리뷰 승인과 전체 자동 검증을 통과한다.
2. 당시 조건: GitHub Ubuntu/Windows CI와 Windows package workflow가 성공한다.
3. 당시 조건: Windows artifact 두 개와 SHA-256, 다운로드 링크가 존재한다.
4. 당시 조건: HVC 기록지의 모든 필수 항목이 실제 증거와 함께 통과한다.
5. 당시 조건: 그 뒤에만 `main`, `v0.1.0`, GitHub Release와 다운로드 링크가 생성된다.
