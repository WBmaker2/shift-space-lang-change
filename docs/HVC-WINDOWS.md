...
## 다음 HVC 대상 — v0.1.3 설치형·포터블형

v0.1.3 구현 후보는 기존 설치형과 `ShiftSpaceLangChange-Portable-0.1.3-x64.zip` 포터블형을 함께 제공합니다. 아래 항목은 Windows 10/11 x64 실기기에서 확인하기 전까지 모두 `미검증`으로 유지합니다. Windows runner의 ZIP 구조·해시 검증은 이 표의 실기기 통과를 대신하지 않습니다.
